...
class Agent:
    ...
    def __init__(self, state_size, action_size):
        ...
        self.min_exp_rate = 0.2
        self.exp_rate_decay = (self.exp_rate - self.min_exp_rate) / 200
    ...
    def online_train(self, state, action, reward, next_state, done):
        ...
        if done:
            ...
            target[0][action] = reward + self.discount_rate * np.amax(self.model.predict(next_state)[0])

        history = self.model.fit(state, target, batch_size=1, epochs=1, verbose=0)
        return history
...
if __name__ == "__main__":
    ...
    for episode in range(1, episode_count + 1):
        ...
        for frame in range(1, 100000):
            ...
            # Train at every frame
            history = agent.online_train(state, action, reward, next_state, done)     
            mape_list.append(history.history['mean_absolute_percentage_error'][0])  
        ...
        if agent.exp_rate > agent.min_exp_rate:
            agent.exp_rate -= agent.exp_rate_decay

        # Print values
        time_elapsed = round(real_time.time() - start_time, 2)
        average_reward = round(sum(reward_list) / frame, 2)
        mape = round(sum(mape_list) / len(mape_list), 2)
